...
export default class RippleEffect extends PureComponent {
    ...
    startRipple(event) {
        ...
        let obj = { locationX: 0, locationY: 0 };
        obj = rippleCentered ? { locationX: w2, locationY: h2 } : event?.nativeEvent;

        let offsetX = Math.abs(w2 - obj?.locationX);
        let offsetY = Math.abs(h2 - obj?.locationY);
